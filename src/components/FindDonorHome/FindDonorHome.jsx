...
import { useTranslations, useLocale } from 'next-intl'; // CHANGED: added useLocale
import { useRouter } from 'next/navigation'; // CHANGED: added router

const FindDonor = () => {
  const t = useTranslations('FindDonor');
  const locale = useLocale(); // CHANGED: get current locale
  ...
  const handleDivisionChange = (e) => {
    ...
    setFormData((prev) => ({
      ...
      district: '' // CHANGED: reset district when division changes
    }));
  };
  ...
  const handleDistrictChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      district: selectedDistrict // CHANGED: separate handler for district
    }));
  };
  ...
  const handleBloodGroupChange = (e) => {
    ...
    setFormData((prev) => ({
      ...prev,
      bloodGroup: selectedBloodGroup // CHANGED: separate handler for blood group
    }));
  };

  const handleSearch = (e) => {
    e.preventDefault(); // CHANGED: prevent page reload
    ...
    if (formData.division) {
      params.set('division', formData.division);
    }
    ...
    // CHANGED: push to find-donor page with query params
    // IMPORTANT: if your route folder name is different, change "find-donor"
    router.push(`/${locale}/find-donors?${params.toString()}`);
  };
  ...
  return (
    <section className="relative z-20 container mx-auto px-4 md:px-8 py-12 lg:py-24 bg-base-200">
      <div className="max-w-6xl mx-auto">
        ...
        >
          {/* CHANGED: use form onSubmit so submit is reliable */}
          <form
          ...
          >
            ...
            <div className="form-control w-full group">
              ...
              <select
                ...
                onChange={handleBloodGroupChange} // CHANGED
                required
              >
            ...
            <div className="form-control w-full group">
              ...
              <select
                ...
                onChange={handleDistrictChange} // CHANGED
            ...
            <div className="form-control w-full mt-4 lg:mt-0">
              <button
                type="submit" // CHANGED: submit button instead of manual click
