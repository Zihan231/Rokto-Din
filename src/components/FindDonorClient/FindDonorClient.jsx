...
export default function FindDonorClient() {
  ...
  const fetchDonors = async (page = 1, searchData = formData) => {
    setIsLoading(true);
  ...
  const handleSearch = () => {
    ...
    params.set('bloodGroup', formData.bloodGroup);

    if (formData.division) params.set('division', formData.division);
  ...
  const openContactModal = (donor) => {
    ...
    if (options.length === 1) {
      ...
      } else if (donor.whatsappNumber) {
        window.open(`https://wa.me/${donor.whatsappNumber}`, '_blank');
      } else if (donor.facebookLink) {
        window.open(donor.facebookLink, '_blank');
      }
    } else {
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                      {results.map((donor) => (
                        <div
                          key={donor.id}
                          className="bg-white rounded-2xl p-6 border border-base-200 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden"
                        >
                          {/* Top Row */}
                          ...
                          {/* Donor Details */}
                          <h4 className="text-xl font-bold text-neutral mb-1">
                            {donor.fullName}
                          </h4>
        ...
        {selectedDonor && (
          <div className="modal modal-open modal-bottom sm:modal-middle backdrop-blur-sm bg-black/30 z-50">
            <motion.div
              ...
              className="modal-box bg-white p-8 rounded-3xl border-t-4 border-primary shadow-2xl"
            >
              <button
                onClick={() => setSelectedDonor(null)}
                className="btn btn-sm btn-circle btn-ghost absolute right-4 top-4 hover:bg-red-50 hover:text-red-500 transition-colors"
              >
                <X size={20} />
              </button>

              <h3 className="text-2xl font-black text-center mb-2 text-neutral">
                {t('modal.title')}
              </h3>
              ...
              <div className="grid grid-cols-1 gap-4">
                {selectedDonor.phoneNumber && (
                  <a
                    ...
                    className="flex items-center justify-center gap-4 p-4 rounded-2xl bg-green-50 text-green-700 border border-green-100 font-bold hover:bg-green-100 transition-colors"
                  >
                    <Phone size={24} />
                    {t('modal.call')}
                ...
                {selectedDonor.whatsappNumber && (
                  <a
                    href={`https://wa.me/${selectedDonor.whatsappNumber}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-4 p-4 rounded-2xl bg-emerald-50 text-emerald-700 border border-emerald-100 font-bold hover:bg-emerald-100 transition-colors"
                  >
                ...
                {selectedDonor.facebookLink && (
                  <a
                    href={selectedDonor.facebookLink}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-4 p-4 rounded-2xl bg-blue-50 text-blue-700 border border-blue-100 font-bold hover:bg-blue-100 transition-colors"
                  >
                    <facebookLink size={24} />
                    {t('modal.facebook')}
                  </a>
                )}
              </div>
